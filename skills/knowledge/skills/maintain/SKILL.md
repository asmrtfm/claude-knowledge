...
!`${CLAUDE_SKILL_DIR}/audit.sh`

## Maintain Mode

Review and update knowledge entries that may be stale.
The maintenance queue is populated automatically by PostToolUse hooks that record file paths touched by Edit, Write, and NotebookEdit tool calls during normal sessions.
...
The `tool_name` field distinguishes between files that were modified (`Edit`) vs newly created (`Write`) — use this to determine whether to check existing entries for staleness or evaluate whether a new file warrants an entry.
...
2. If the queue is empty, do a broader scan:
   a. Sort entries by the updated timestamp in their frontmatter (do NOT read the entire entry just to get its frontmatter). Perform all subsequent steps per entry in sorted order, never batched.
   b. For each, verify referenced files exist and knowledge still applies
   c. Flag any with missing files, obviously stale content, or that are covering two topics that could be split into smaller dedicated entries for better orgnization.
   d. Report your findings and ask wait for the user to tell you waht to do next.
   e. Repeat this process for the next entry if the user confirms you should proceed.
